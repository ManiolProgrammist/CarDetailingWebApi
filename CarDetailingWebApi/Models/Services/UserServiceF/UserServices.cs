...
namespace CarDetailingWebApi.Models
{
   ...
   public class UserServices : IUserService
   {
      ...
      //function that remove password from returned informations
      ...
      public Result<User> ChangePassword(string login, string oldPassword, string newPassword)
      {
         var r = this._userRepository.Login(login, oldPassword);
         if (r.status == true)
         {
            var user = r.value;
            ScryptEncoder encoder = new ScryptEncoder();
            user.Password = encoder.Encode(newPassword);
            r = this._userRepository.Update(user);
         }
         return retWithoutPassword(r);
      }
   }
